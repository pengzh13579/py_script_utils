...
class MessageBox(QtWidgets.QWidget):  # 继承自父类QtWidgets.QWidget
    ...
    def __init__(self, parent=None):  # parent = None代表此QWidget属于最上层的窗口,也就是MainWindows.
        ...
        self.setWindowTitle(u'爱你')  # 仅仅设置窗体标题，不设置位置。
        ...
        self.label1 = QtWidgets.QLabel(u'<b>王晓猪，观察你很久了！</b>', self)  # 建立一个标签
        self.label1.move(0, 40)  # 使此标签移动到这个部件(260,40)的位置
        self.label1.setFont(QFont("Timers", 20));  # 设置字体与字体大小
        self.label2 = QtWidgets.QLabel(u'<b>做我老婆好不好?</b>', self)  # 建立一个标签
        ...
        # Qt::WheelFocus StrongFocus | 0x4  滑轮作为焦点选中事件
        # Qt::NoFocus 0 不接受焦点
        self.buttonE.move(100, 200)  # move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
    ...
    def showDialogNO(self):
        self.q = random.randint(0, 200)  # 在0-650内生成随机的X坐标
        self.w = random.randint(0, 200)  # 在150-650内生成随机的Y坐标
    ...
    def closeEvent(self, event):  # 函数名固定不可变
        ...
        if self.CloseAllowed == 1:
            ...
            QtWidgets.QMessageBox.information(self, "未作回应", "王晓猪,请不要逃避!", QtWidgets.QMessageBox.Ok)
